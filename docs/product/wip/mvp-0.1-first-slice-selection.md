...
Source-monotonic time defines fixture order and source semantics; AirLink-observed delivery time remains separate. Start begins delivery, Pause freezes delivery progression without lifecycle meaning, and `1×`/`2×` scale delivery intervals without altering frozen timestamps or values. C10 Reset resets replay state to the beginning of the stream only. The selected slice keeps Reset unavailable during an active Flight; resumed issue #37 must define the cross-concern handoffs that create a fresh development session or discard the C9 in-memory record.

Replay events use the Engineering Map's stable composite identity `streamId + sourceCategory + sourceMonotonicTime + equalTimeSequence`. Batching and delay affect delivery only. Exact redelivery preserves that identity and the complete payload and metadata and is handled idempotently. The same identity with different value or metadata is a collision and fails closed for the affected stream. Injected invalidity or availability transitions receive the same explicit identity fields and deterministic ordering. Transforms remain explicit and diagnostic; they cannot create ambiguous events, recalculate values, invent baseline cadence/error behavior, or expose Generator truth.
