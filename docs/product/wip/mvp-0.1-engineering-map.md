...
3. mixed-source runs are required when each value retains live, selected, pass-through, or simulated category-level provenance and the requested and actually active source configurations remain explicit;
...
Normal end-to-end simulation substitutes or controls approved external input production and uses the same downstream concerns as live input. C10 owns whether its controlled simulation run is active, the requested source composition, and controlled simulated-source production. Live and pass-through runtime values may reach C4 directly, and live weather may reach C5 directly; C10 does not proxy them. C4 and C5 own the actually active AirLink-facing source state, availability, validity, freshness, and category-level provenance within their respective boundaries.

Every Flight created while a C10-controlled simulation run is active is classified as simulated regardless of the run's source composition. C10 supplies the active run context when C3 creates the Flight; C3 associates the Flight-level simulation classification with Flight identity; C9 preserves the supplied classification durably; and C1 presents it. A Flight created outside a C10-controlled simulation run does not become simulated merely because an input is selected or has another unusual provenance. Category-level input provenance remains independently preserved and is not collapsed into, or used by C9 to infer, Flight-level classification.
...
- availability, validity, freshness, accuracy or quality metadata, and live, selected, pass-through, or simulated category-level provenance;
...
- requested simulation-run source configuration from C10 where it affects C4-owned categories;
...
- actually active source, activation, availability, validity, freshness, and provenance outcomes for consumers and C10 observability.
...
- actually active weather-source interpretation, availability, freshness, validity, live, selected, pass-through, or simulated category-level provenance, and degraded state.
...
- requested simulation-run weather-source configuration from C10;
...
- actually active weather-source and activation outcomes for C10 observability.
...
- C10 simulation-run state or requested source composition.

C5 applies the same observation-versus-forecast, source-time, AirLink-observed-time, validity, freshness, category-level provenance, and degraded-state meaning to live, selected, pass-through, or simulated weather. C10 may request weather-source composition and may supply a controlled simulated equivalent, but C5 owns the actually active weather interpretation; live weather need not pass through C10. Simulated weather does not create a second Weather Context concern or provider-specific product behavior.
...
C7 preserves the source dependencies and semantic status required to understand derived output. A derived value may depend on multiple sources, each of which retains its own live, selected, pass-through, or simulated category-level provenance. Those dependencies must not be silently presented or retained as if every contributing source were live, source substitution must not select an alternative calculation meaning, and C3-supplied Flight-level simulation classification remains a separate semantic axis.
...
C8 uses the same spatial semantics regardless of live, selected, pass-through, or simulated category-level input provenance and regardless of single-source or mixed-source run composition. A selected starting point or route may constrain source production for validation, but it does not enable Active Navigation, transfer C8 state ownership to C10, or create a simulation-only map interpretation. C3-supplied Flight-level simulation classification remains separate from spatial input provenance.
...
- category-level source and derivation provenance supplied through the normal C4/C7 recording inputs;
...
- Flight-level simulation classification or inference from recorded source composition;
- current-value calculations or later reinterpretations;
- presentation.

C9 must preserve the Flight-level simulation classification supplied by C3 and must not derive, infer, or reclassify it from the recorded input composition or category-level provenance. One physical store or separate stores, exact metadata, history filtering, visual marking, grouping, default retention, automatic cleanup, storage technology, and deletion mechanics remain deferred.
...
- requested live, selected, pass-through, or simulated source composition for the run;
...
- explicit simulated category-level provenance for C10-produced values;
...
- availability or activation outcomes for approved pass-through sources;
- relevant C4/C5 actually active source-state outcomes needed to expose the run configuration;
- deletion outcomes from C9 when cleanup is requested.

**Produces**

- requested source configuration for C4- and C5-owned categories;
...
- requested source configuration, run identity, scenario state, and progress for observability;
...
- acquisition, normalization, or actually active runtime-source configuration owned by C4;
- weather-provider integration or actually active weather-source interpretation owned by C5;
...
C10 is not an alternative detector, calculation system, recorder, lifecycle, application, or third product pillar, and live or pass-through values do not need to be proxied through it. Exact controls, scenario representation, operator workflow, source-selection realization, architecture, automation, implementation technology, and tooling remain deferred.

## Live-input and conceptual-substitution categories

The table defines category-level boundaries, not APIs, sensor selection, fusion, sampling, or source hierarchy. Unless stated otherwise, the conceptual substitution point is external production before C4 normalization. C10 requests the run's source composition and controls simulated production, while live and pass-through sources may reach C4 or C5 directly. C4 and C5 own the actually active AirLink-facing result and apply the same validity and category-level provenance treatment regardless of production; downstream concerns do not receive a simulation-only path.
...
An individual value has live, selected, pass-through, or simulated category-level provenance; `mixed` is not value-level provenance. A run or actually active source configuration is mixed-source when relevant categories use more than one of those provenance types. Mixed-source runs may include live weather with simulated movement, live current location with simulated movement, a selected starting point or route with simulated movement, live map capability with simulated spatial inputs, or live clocks and platform context with simulated Flight inputs. C10's requested composition, C4/C5's actually active source state, and category-level provenance must be separately observable, and switching must not occur invisibly. A Flight created while the C10-controlled run is active is simulated regardless of that mix; a Flight outside such a run is non-simulated regardless of an individual value's provenance. Exact compatibility rules and source-selection mechanics remain deferred.

## Source-independent downstream contract

Live, selected, pass-through, or simulated category-level production, in either single-source or mixed-source run composition, must preserve the normal responsibilities below. Source quality may legitimately change availability or degraded behavior, but it does not change the product meaning of a concern.
...
| C4 — Input Acquisition and Validity | Normalizes every approved runtime source, preserves category-level provenance and time semantics, owns the actually active configuration for its categories, and exposes activation, validity, freshness, availability, and degradation outcomes |
| C5 — Weather Context | Owns the actually active weather-source interpretation and preserves normal weather meaning, location scoping, observation-versus-forecast distinction, category-level provenance, freshness, and degradation regardless of production |
...
| C9 — Flight Recording and Local Retention | Uses the normal progressive recording, finalization, retention, retrieval, and deletion responsibilities while durably preserving C3-supplied Flight-level simulation classification without inferring it from source composition |
...
- C10-requested source configuration, whether the C10-controlled simulation run is active, conceptual run or scenario identity, state, and progress;
- C4/C5 actually active source configuration and activation outcomes, live, selected, pass-through, or simulated category-level provenance, observable single-source or mixed-source composition, and any selected starting point or route context;
...
| O6 | Normalized runtime inputs; source and observed time; wall-clock and monotonic semantics; availability, validity, freshness, quality, category-level provenance, and actually active configuration for C4-owned categories | C4 | C5 consumes current-location context; runtime consumers use required inputs without reclassifying or silently switching source state; C10 consumes activation outcomes to expose requested-versus-active configuration |
| O7 | Current-location weather acquisition demand, actually active weather-source interpretation, weather observation, forecast, pressure or QNH context, category-level provenance, and degradation | C5 | C4 fulfills the location demand; C1 presents weather; C7 conditionally consumes pressure or QNH; C10 consumes activation outcomes; live and simulated weather retain the same product meaning |
...
| O12 | Authoritative C10 simulation-run active state, requested source composition, approved simulated-source production, selected-source controls, simulated-time control, scenario state, and progress | C10 | C3 consumes active run context when creating a Flight; C4/C5 receive requested configuration and controlled equivalents while owning actual active source state; C10 consumes their activation outcomes for observability; C2–C9 retain normal responsibility ownership |
...
| --- | --- | --- | --- | --- |
| F1 | Current conditions | C1 requests current conditions from C5; C5 expresses bounded current-location acquisition demand to C4; C4 fulfills that demand and supplies normalized current-location context, including availability, validity, freshness, and provenance, to C5; C5 uses that context to scope weather interpretation and receives either external live-provider information directly or a C10-controlled simulated weather equivalent through the same weather-input boundary; C5 supplies observed weather, forecast when available, observation-versus-forecast distinction, freshness, validity, category-level provenance, and degraded state to C1 and exposes the actually active weather-source outcome to C10 when relevant | The pilot can understand relevant conditions for the current location before Pre-Flight through the same C5 semantics for live or simulated weather; unavailable or invalid location or weather produces an explicit limitation rather than silently using an unrelated location or source | Provider, request mechanism, concurrent-demand coordination, refresh/cache policy, forecast intervals, exact representation, fallback details |
...
Measured, declared, recorded, estimated, and derived information remain distinguishable where their meaning matters. An individual value retains live, selected, pass-through, or simulated category-level provenance; mixed-source describes run or configuration composition rather than value provenance. Source and observed time, wall-clock and monotonic semantics, validity, freshness, availability, quality, and degradation must not be inferred from hidden implementation details. Source switching is explicit, and multiple provenance dependencies are independently preserved during derivation and retention wherever source identity affects meaning. Flight-level simulated or non-simulated classification remains separate and follows the C10-run-to-C3 association rule.
...
A Flight created while a C10-controlled simulation run is active is classified as simulated regardless of the run's input composition. C3 associates that classification with Flight identity at creation; the Flight then follows the normal recording, completion, Summary, retention, retrieval, and saved-review paths while remaining durably distinguishable from a non-simulated Flight. C9 preserves but never infers or redefines the classification. C9 owns deletion of individual or all simulated Flights; a category-wide simulated-Flight deletion must not delete non-simulated Flights. Category-level input provenance remains independently preserved.
...
10. mixed-source composition remains observable without treating `mixed` as individual-value provenance, and C10-requested configuration remains distinct from C4/C5 actually active source state;
...
15. mandatory observability covers requested and actually active source configuration, category-level provenance, run composition, Flight-level classification, inputs, lifecycle, detection, derivation, spatial results, recording, retention, deletion, Summary, and saved review;
...
- every required equivalent enters the normal C4 or C5 boundary, C10-requested source composition remains distinct from C4/C5 actually active source state, and live or pass-through values need not be proxied through C10;
- each value retains live, selected, pass-through, or simulated category-level provenance while mixed-source remains a run/configuration property and multiple derivation dependencies remain independent;
...
- every Flight created during an active C10-controlled simulation run is classified as simulated regardless of source composition through an explicit C10 → C3 → C9 → C1 path, while a Flight outside such a run is not classified from individual input provenance;
- C9 preserves C3-supplied Flight-level classification without deriving, inferring, or redefining it;
