...
WIP is not loaded by default. Product policy is also not loaded by default: the Product-Significance Routing rule below requires consultation of `docs/product/policy/ProductGovernance.md` and `docs/product/policy/ProductDirection.md` only when a trigger applies. WIP may be consulted only when the task explicitly concerns it or when it is directly relevant to the affected product decision and the task, issue, or triggered alignment process identifies why it is needed. Consultation does not make WIP canonical or make policy or WIP an implementation requirement.
...
The repository-wide obligation comes from this active rule. Product Governance is an accepted conditional product-policy artifact invoked through this routing gate, and Product Direction is an accepted product-policy artifact consulted as alignment context. Neither document is an implementation requirement. Mandatory consultation does not let them override canon, an approved iteration, approved issue scope, or an explicit owner decision. Identify exact conflicting statements and their authority; do not invent missing product behavior or implement a guessed compromise.
